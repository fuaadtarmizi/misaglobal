...
export default function App() {
  ...
  function Submit(e) {
    ...
    fetch(
      "https://script.google.com/macros/s/AKfycbykw2vgchXpYbA6MfLOTMMDUfZnRkjbNsu9AqlTXH1Q9PJcuMyuxqGwrh7SnzEVJMkUug/exec",
      {
        method: "POST",
        body: formData
      }
          ...
          {isOpen4 && (
            ...
            {isOpen2 && (
              <div className="w-full right-0 mt-2  origin-top-right bg-white border border-gray-200 divide-y  rounded-md shadow-lg ring-1 ring-black ring-opacity-5">
                ...
                <div className=" text-sm bg-yellow-500">
                <p className="px-4 py-2 cursor-pointer hover:bg-yellow-400 rounded-md"
                  onClick={() => handleItemClick2("Butterworth(Main)")}
                >
                  Butterworth(Main)
                </p>
                <p className="px-4 py-2 cursor-pointer hover:bg-yellow-400 rounded-md"
                  onClick={() => handleItemClick2("Bayan Lepas")}
                >
                  Bayan Lepas
                </p>
                </div>     
